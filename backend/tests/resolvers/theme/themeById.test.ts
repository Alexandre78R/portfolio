...
describe("ThemeResolver - getThemeById", () => {
  ...
  it("should return theme for admin ignoring visibility", async () => {
    ...
    const result: ThemeResponse = await resolver.getThemeById(1, adminCtx);
  ...
  it("should return theme only if visible for non-admin", async () => {
    ...
    const result: ThemeResponse = await resolver.getThemeById(1, userCtx);
    expect(result.code).toBe(200);
  ...
  it("should return 404 if theme not found", async () => {
    ...
    const result: ThemeResponse = await resolver.getThemeById(999, adminCtx);
