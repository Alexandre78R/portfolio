...
describe("EducationResolver - updateEducation", () => {
  ...
  it("should successfully update an education record by an admin user", async (): Promise<void> => {
    ...
    const mockUpdatedEducation: PrismaEducation = { ...mockExistingEducation, ...updateInput };
  ...
  it("should return 401 if no user is authenticated", async (): Promise<void> => {
    ...
    const unauthContext: MyContext = { ...baseMockContext, user: null };
    const updateInput: UpdateEducationInput = { id: 1, titleEN: "Test" };

    const result: EducationResponse = await resolver.updateEducation(updateInput, unauthContext);
  ...
  it("should return 403 if user is not admin or editor", async (): Promise<void> => {

    const userContext: MyContext = { ...baseMockContext, user: mockRegularUser };
    const updateInput: UpdateEducationInput = { id: 1, titleEN: "Test" };

    const result: EducationResponse = await resolver.updateEducation(updateInput, userContext);
  ...
  it("should return 500 if there is a database error during findUnique", async (): Promise<void> => {
  ...
  it("should return 500 if there is a database error during update", async (): Promise<void> => {

    const adminContext: MyContext = { ...baseMockContext, user: mockAdminUser };
    const updateInput: UpdateEducationInput = { id: mockExistingEducation.id, titleEN: "Test" };
    prismaMock.education.findUnique.mockResolvedValueOnce(mockExistingEducation);
    prismaMock.education.update.mockRejectedValueOnce(new Error("DB error during update"));
