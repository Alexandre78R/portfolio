const HomeTerminal: React.FC = (): React.ReactElement => {
  const textResponsive: (text: string) => string | undefined = (
    text: string
  ): string | undefined => {
    if (text == "visitor") {
  ...
  return (
    <div className="inline-block mr-2">
      <span className="text-secondary">{textResponsive("visitor")}</span>@
      <span className="text-primary">{textResponsive("domaine")}</span> :~$
    </div>
  );
};
